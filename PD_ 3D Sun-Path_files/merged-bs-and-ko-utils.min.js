        ...
        a((e = Object.assign({}, t.snackbar.defaults, e)).id)
          ? t("#" + e.id).length
            ...
            : ((s = t("<div/>")
                ...
                .attr("class", "snackbar")),
              (i = !0))
          : ((e.id = "snackbar" + Date.now()),
            (s = t("<div/>").attr("id", e.id).attr("class", "snackbar")),
            (i = !0));
        var o = s.hasClass("snackbar-opened");
        a(e.style)
          ? (o
              ? s.attr("class", "snackbar snackbar-opened " + e.style)
              : s.attr("class", "snackbar " + e.style),
            s.attr("data-style", e.style))
          : o
          ...
          (e.content = e.htmlAllowed
            ? e.content
            : t("<p>" + e.content + "</p>").text()),
          a(e.onClose) && n.on(e.id, e.onClose),
          a(e.htmlAllowed) && s.attr("data-html-allowed", e.htmlAllowed),
          a(e.content) &&
            (s.find(".snackbar-content").length
              ? s.find(".snackbar-content").html(e.content)
              : s.prepend(
                  "<span class=snackbar-content>" + e.content + "</span>"
                ),
            s.attr("data-content", e.content)),
          i
            ? s.appendTo("#snackbar-container")
            : s.insertAfter("#snackbar-container .snackbar:last-child"),
          a(e.action) &&
            "toggle" == e.action &&
            (e.action = o ? "hide" : "show");
        var c = Date.now();
        s.data("animationId1", c),
          setTimeout(function () {
            s.data("animationId1") === c &&
              (a(e.action) && "show" != e.action
                ? a(e.action) &&
                  "hide" == e.action &&
                  (s.removeClass("snackbar-opened"), n.emit(e.id), n.off(e.id))
                : s.addClass("snackbar-opened"));
          }, 50);
        var r = Date.now();
        return (
    ...
    (t.fn.snackbar = function (n) {
      if (void 0 !== n) {
        ...
        (a(n) && "show" !== n && "hide" !== n && "toggle" != n) ||
          ...
          a(n) &&
            ((e.id = this.attr("data-snackbar-id")),
            ("show" !== n && "hide" !== n && "toggle" != n) || (e.action = n));
        var s = t.snackbar(e);
...
!(function (e, t) {
  ...
  (e.utils.uniqueId =
    ...
    (n = {
      alert:
        '<div class="alert fade in" data-bind="css: type, template: innerTemplate"> </div>',
      alertInner:
        '<button class="close" data-dismiss="alert" aria-hidden="true">&times;</button> <p data-bind="text: message"></p>',
      carousel:
      ...
      modalHeader:
        '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button> <h3 data-bind="text: label"></h3> ',
      progress:
    ...
    (e.bindingHandlers.alert = {
      ...
      update: function (a, n, i, r, o) {
        ...
        if (a.nodeType === (Node.ELEMENT_NODE || 1))
          (s = u),
            (d = p.data || { message: p.message }),
            l
              .addClass("alert fade in")
              .addClass("alert-" + (e.unwrap(p.type) || "info"));
        else {
          if (a.nodeType !== (Node.COMMENT_NODE || 8))
            throw new Error(
              "alert binding should be used with dom elements or ko virtual elements"
            );
          (s = "alert"),
            (d = {
              innerTemplate: {
        ...
        e.renderTemplate(
          s,
          o.createChildContext(d),
          e.utils.extend({ templateEngine: c }, p.templateOptions),
          a
        );
      },
    }),
    ...
    (e.bindingHandlers.carousel = {
      ...
      init: function (a, n, i, r, o) {
          ...
          p = function (e, a) {
            var n = {
              ...
              data:
                (d[a] &&
                  (d[a].data ||
                    (d[a].dataConverter && d[a].dataConverter(d)))) ||
                e.dataConverter(d),
            };
            return (
              (n = t.extend(!0, {}, n, d[a])),
        ...
        a.id
          ...
          : d.id
          ? (a.id = e.unwrap(d.id))
          : (a.id = d.id = e.utils.uniqueId("ks-carousel-"));
        var c = {
          id: d.id,
    ...
    (e.bindingHandlers.checkbox = {
      init: function (a, n) {
        ...
        if (
          "buttons" === i.attr("data-toggle") &&
          i.find("input:checkbox").length
        ) {
          if (!(e.unwrap(n()) instanceof Array))
            throw new Error(
              "checkbox binding should be used only with array or observableArray values in this case"
            );
          i.on("change", "input:checkbox", r);
        } else {
          if ("checkbox" !== i.attr("type"))
            throw new Error(
              "checkbox binding should be used only with bootstrap checkboxes"
            );
          if (!e.isObservable(n()))
            throw new Error(
              "checkbox binding should be used only with observable values in this case"
            );
          i.on("change", r);
        }
      },
    ...
    (e.bindingHandlers.modal = {
      defaults: {
        ...
        footerTemplate: {
          name: "modalFooter",
          templateEngine: e.stringTemplateEngine.instance,
          data: { closeLabel: "Close", primaryLabel: "Ok" },
        },
      },
      init: function (a, n, i, r, o) {
        var s = t(a),
        ...
        if (!d.header || !d.body)
          throw new Error(
            "header and body options are required for modal binding."
          );
        ...
        var u = {
          ...
          footerTemplate: d.footer
            ? c(l.footerTemplate, e.unwrap(d.footer))
            : null,
        };
        return (
          e.renderTemplate(
            "modal",
            o.createChildContext(u),
            { templateEngine: e.stringTemplateEngine.instance },
            a
          ),
      ...
      update: function (a, n) {
        var i = n();
        void 0 !== i.visible &&
          t(a).modal(e.unwrap(i.visible) ? "show" : "hide");
      },
    });
  ...
  (e.bindingHandlers.popover = {
    ...
    update: function (a, n, r, o, s) {
      var d = t(a),
        l = e.unwrap(n()),
        p =
          (l.options || l.template
            ? e.utils.unwrapProperties(l.options)
            : e.utils.unwrapProperties(l)) || {};
      if (l.template) {
        e.unwrap(l.template);
        var c = e.utils.domData.get(a, i),
          ...
          m = function () {
            e.renderTemplate(
              e.unwrap(l.template),
              s.createChildContext(u),
              l.templateOptions,
              document.getElementById(c)
            );
            var a = t("#" + c).parents(".popover"),
              n = d.data("bs.popover"),
              i = n.getCalculatedOffset(
                p.placement || "right",
                n.getPosition(),
                a.outerWidth(),
                a.outerHeight()
      ...
      v
        ...
        : (d.popover(p),
          d.on("shown.bs.popover", function () {
            (p.container ? t(p.container) : d.parent()).one(
              "click",
      ...
      init: function (a, n) {
        ...
        if ("number" == typeof o)
          ...
          if ("number" != typeof e.unwrap(o.value))
            throw new Error(
              'progress binding can accept only numbers or objects with "value" number propertie'
            );
          d.barWidth = e.computed(function () {
    ...
    (e.bindingHandlers.radio = {
      init: function (a, n) {
        if (!e.isObservable(n()))
          throw new Error(
            "radio binding should be used only with observable values"
          );
      ...
      update: function (a, n) {
        ...
        r.length
          ? ((i = r.parent()).siblings().removeClass("active"),
            i.addClass("active"),
            r.prop("checked", !0))
          : ((i = t(a).find(".active")).removeClass("active"),
    ...
    (e.bindingHandlers.toggle = {
      init: function (a, n) {
        ...
        if (!e.isObservable(i))
          throw new Error(
            "toggle binding should be used only with observable values"
          );
  ...
  (e.prototype = t.extend({}, t.fn.modal.Constructor.prototype, {
    ...
    init: function () {
      ...
      (e.$body = t(document.body)),
        ...
        (e.useOffsetForPos =
          void 0 == e.options.useOffsetForPos || e.options.useOffsetForPos),
        e.$element.find(".popover-footer").length &&
          e.$element.removeClass("has-footer").addClass("has-footer"),
        e.options.remote &&
          e.$element
            .find(".popover-content")
            .load(e.options.remote, function () {
    ...
    show: function () {
      var e = this,
        o = e.$element;
      o.css({ top: 0, left: 0, display: "block", "z-index": 1050 }),
        e.refreshPosition(),
      ...
      return this.each(function () {
        ...
        if (
          (i.$target ||
            (a && a.$target
              ? (i.$target = a.$target)
              : (i.$target = o.$target || t(o.target))),
          a)
        ) {
          if ("object" == typeof o && o)
            for (var p in ("placement" in o &&
              r.removeClass(a.options.placement),
            o))
    ...
    t(document).on("ready", function () {
      t("[data-toggle='popover-x']").on("click", function (e) {
        var o = t(this),
          r = o.attr("href"),
          a = t(
            o.attr("data-target") || (r && r.replace(/.*(?=#[^\s]+$)/, ""))
          ),
          i = a.data("popover-x")
            ? "toggle"
            : t.extend({ remote: !/#/.test(r) && r }, a.data(), o.data());
        e.preventDefault(),
        ...
        t('[data-toggle="popover-x"]').on("keyup", function (e) {
          ...
          r &&
            27 == e.which &&
            r.trigger("keyup.target.popoverX") &&
            r.popoverX("hide");
        });
    });
})(window.jQuery);
